...
def load_income():
    return pd.read_sql_query("SELECT id, income_date, source, description, amount FROM income ORDER BY income_date DESC, id DESC", conn)


def load_categories():
    return [r[0] for r in conn.execute("SELECT name FROM categories ORDER BY id").fetchall()]
...
def update_expense(expense_id, expense_date, category, description, amount):
    conn.execute("UPDATE expenses SET expense_date=?, category=?, description=?, amount=? WHERE id=?", (expense_date.isoformat(), category, description.strip(), amount, expense_id))
...
def save_category_budget(category, amount):
    if amount > 0:
        conn.execute("INSERT INTO category_budgets(category, amount) VALUES(?, ?) ON CONFLICT(category) DO UPDATE SET amount=excluded.amount", (category, amount))
    else:
...
if dark:
    app_bg, card_bg, text, muted, border, plot_bg = "#0b1120", "#111827", "#f9fafb", "#cbd5e1", "#374151", "#111827"
    input_bg = "#1f2937"
else:
    app_bg, card_bg, text, muted, border, plot_bg = "#f5f7fb", "#ffffff", "#111827", "#4b5563", "#e5e7eb", "#ffffff"
    input_bg = "#ffffff"
...
/* Main-page text: explicitly follow the selected theme */
.main *, section.main, section.main p, section.main span, section.main label,
section.main h1, section.main h2, section.main h3, section.main h4,
section.main h5, section.main h6, section.main [data-testid="stMarkdownContainer"] {{ color:{text}; }}
.main .stCaption, section.main [data-testid="stCaptionContainer"] {{ color:{muted} !important; }}

/* Sidebar stays readable in both modes */
[data-testid="stSidebar"] {{ background:#111827 !important; }}
[data-testid="stSidebar"] *, [data-testid="stSidebar"] label,
[data-testid="stSidebar"] p, [data-testid="stSidebar"] span,
[data-testid="stSidebar"] [data-testid="stMarkdownContainer"] {{ color:#f9fafb !important; }}
...
section.main [data-baseweb="textarea"] > div {{ background:{input_bg} !important; color:{text} !important; border-color:{border} !important; }}
...
.hero p {{ margin:7px 0 0; color:#e5e7eb !important; }}
.card {{ background:{card_bg}; border:1px solid {border}; border-radius:18px; padding:19px; box-shadow:0 5px 18px #0f172a18; min-height:115px; }}
.label {{ font-size:.82rem; color:{muted} !important; font-weight:650; }}
.value {{ font-size:1.55rem; color:{text} !important; font-weight:750; margin-top:7px; }}
.section {{ font-size:1.18rem; font-weight:750; color:{text} !important; margin:22px 0 12px; }}
.insight {{ background:{card_bg}; border:1px solid {border}; border-radius:16px; padding:15px 17px; height:100%; color:{text} !important; }}
.insight * {{ color:{text} !important; }}
.insight small {{ color:{muted} !important; }}

/* Dataframe/table */
section.main [data-testid="stDataFrame"] {{ border:1px solid {border} !important; }}
section.main [data-testid="stDataFrame"] * {{ color:{text} !important; }}
...
/* Native alerts */
section.main [data-testid="stAlert"] {{ color:{text} !important; }}
</style>
...
with st.sidebar:
    ...
    with st.form("expense_form", clear_on_submit=True):
        ...
        if st.form_submit_button("Add Expense  →", use_container_width=True, type="primary"):
            ...
            if category:
                conn.execute("INSERT OR IGNORE INTO categories(name) VALUES(?)", (category,))
                conn.execute("INSERT INTO expenses(expense_date,category,description,amount) VALUES(?,?,?,?)", (expense_date.isoformat(), category, description.strip(), amount))
                conn.commit()
    ...
    with st.form("income_form", clear_on_submit=True):
        ...
        if st.form_submit_button("Add Income  →", use_container_width=True):
            conn.execute("INSERT INTO income(income_date,source,description,amount) VALUES(?,?,?,?)", (income_date.isoformat(), source, income_description.strip(), income_amount))
            conn.commit()
