...
.stButton button:active,.stDownloadButton button:active,[data-testid="stFormSubmitButton"] button:active{transform:translateY(1px) scale(.97)!important;box-shadow:0 2px 6px rgba(15,23,42,.12)!important}
.stButton button:focus-visible,.stDownloadButton button:focus-visible,[data-testid="stFormSubmitButton"] button:focus-visible{outline:3px solid rgba(99,102,241,.28)!important;outline-offset:2px!important}
...
[data-testid="stSegmentedControl"] button{transition:all .18s ease!important}[data-testid="stSegmentedControl"] button:hover{transform:translateY(-1px)!important}
.stDownloadButton button{background:white!important;color:#374151!important}
.delete-row button{background:#fff1f2!important;color:#dc2626!important;border-color:#fecdd3!important}.delete-row button:hover{background:#dc2626!important;color:white!important;border-color:#dc2626!important}
</style>
...
df=load_expenses(); today=date.today(); total=float(df.amount.sum()) if not df.empty else 0
month_df=df[pd.to_datetime(df.expense_date).dt.to_period("M")==pd.Period(today, freq="M")] if not df.empty else df
month_total=float(month_df.amount.sum()) if not month_df.empty else 0; remaining=max(budget-month_total,0); progress=min(month_total/budget,1) if budget>0 else 0
...
if df.empty: st.info("No expenses yet. Add your first expense from the sidebar."); st.stop()

st.markdown('<div class="section">🎯 Monthly Budget</div>',unsafe_allow_html=True); st.progress(progress)
status="Within budget" if month_total<=budget else "Budget exceeded"; st.caption(f"{status} • ₹{month_total:,.2f} of ₹{budget:,.2f} used ({progress*100:.0f}%)")
st.markdown('<div class="section">📊 Expense Analytics</div>',unsafe_allow_html=True)
period=st.segmented_control("View by",["Daily","Weekly","Monthly","Yearly"],default="Monthly"); period=period or "Monthly"
...
with left:
    fig=px.bar(grouped,x="Period",y="amount",title=f"{period} Spending",labels={"amount":"Amount (₹)","Period":"Period"},text_auto=".2s"); fig.update_layout(height=390,margin=dict(l=10,r=10,t=55,b=10),paper_bgcolor="white",plot_bgcolor="white"); st.plotly_chart(fig,use_container_width=True)
with right:
    cat=df.groupby("category",as_index=False).amount.sum().sort_values("amount",ascending=False); fig2=px.pie(cat,names="category",values="amount",title="Spending by Category",hole=.48); fig2.update_layout(height=390,margin=dict(l=10,r=10,t=55,b=10),showlegend=True); st.plotly_chart(fig2,use_container_width=True)
...
fig3=px.line(daily,x="expense_date",y="amount",markers=True,title="Daily Spending Trend",labels={"expense_date":"Date","amount":"Amount (₹)"}); fig3.update_layout(height=330,margin=dict(l=10,r=10,t=55,b=10),paper_bgcolor="white",plot_bgcolor="white"); st.plotly_chart(fig3,use_container_width=True)

st.markdown('<div class="section">💡 Smart Insights</div>',unsafe_allow_html=True)
cat_month=month_df.groupby("category").amount.sum() if not month_df.empty else pd.Series(dtype=float); top_cat=cat_month.idxmax() if not cat_month.empty else "No data"; top_val=float(cat_month.max()) if not cat_month.empty else 0
prev_period=today.replace(day=1)-timedelta(days=1); prev_start=prev_period.replace(day=1); prev_df=df[(pd.to_datetime(df.expense_date).dt.date>=prev_start)&(pd.to_datetime(df.expense_date).dt.date<=prev_period)]; prev_total=float(prev_df.amount.sum()) if not prev_df.empty else 0; change=((month_total-prev_total)/prev_total*100) if prev_total else None; avg_daily=month_total/today.day if month_total else 0
ins=st.columns(3)
...
with ins[2]:
    text="No previous-month data" if change is None else (f"↑ {change:.1f}% vs last month" if change>0 else f"↓ {abs(change):.1f}% vs last month"); st.markdown(f'<div class="insight">🔄 <b>Monthly comparison</b><br>{text}<br><small>Previous: ₹{prev_total:,.2f}</small></div>',unsafe_allow_html=True)

st.markdown('<div class="section">🧾 Recent Transactions</div>',unsafe_allow_html=True); st.markdown("<div style='color:#6b7280;font-size:.85rem;margin-bottom:8px'>Click 🗑️ Delete on any transaction to remove it.</div>",unsafe_allow_html=True)
header=st.columns([1.2,1.3,2.5,1.3,.8])
for col,text in zip(header,["Date","Category","Description","Amount","Action"]): col.markdown(f"**{text}**")
for row in df.head(15).itertuples():
    cols=st.columns([1.2,1.3,2.5,1.3,.8]); cols[0].write(row.expense_date); cols[1].write(row.category); cols[2].write(row.description or "—"); cols[3].write(f"₹{row.amount:,.2f}")
    if cols[4].button("🗑️",key=f"delete_{row.id}",help=f"Delete expense #{row.id}",type="secondary"): delete_expense(int(row.id)); st.toast("Expense deleted successfully!",icon="🗑️"); st.rerun()
